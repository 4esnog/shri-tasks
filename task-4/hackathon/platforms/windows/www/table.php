		...
		<thead>
			<tr>
				<td colspan="7">
					...
					Время регистрации в таблице меньше реального на 3 часа.
				</td>
			</tr>
		</thead>
		<tbody>
			...
			while($row = $result->fetch_assoc()) {
				...
				// $t = getdate($row['reg_time']);
				// $time = "$t['mday']";
				// $time = date(DATE_W3C, $row["reg_time"]);
				$true_timestamp = strtotime($row["reg_time"]);
				$time = date('d.m.Y<b\r>H:i:s', $true_timestamp);
