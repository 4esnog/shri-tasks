...
var app = {
	...
	initialize: function initialize() {
		app.debug.append('Init<br />');

		this.bindEvents();
	},
	bindEvents: function bindEvents() {
		app.debug.append('Bind Events<br />');
		app.clientWidth = getComputedStyle(document.documentElement).width.slice(0, -2);

		document.addEventListener('deviceready', this.onDeviceReady, false);
		if (app.clientWidth < 1310) {
			$('.day').addClass('hidden');
			$('.day-title').click(app.switchSchedule.bind(app));
		}
		$('.registration-btn').click(app.scrollToRegister.bind(app));
		$(window).scroll(app.onWindowScroll.bind(app));
		$('#register-btn').click(app.register.bind(app));
		window.onresize = app.resizeVideo.bind(app);
	},
	onDeviceReady: function onDeviceReady() {
		app.debug.append('Device ready!<br />');

		// $.mobile.allowCrossDomainPages = true;
		$.support.cors = true;

		app.request = indexedDB.open('AppDB');
		app.request.onerror = app.onIDBRequestError;
		app.request.onsuccess = app.onIDBRequestSuccess;

		app.resizeVideo();
	},
	onIDBRequestError: function onIDBRequestError(e) {
		app.debug.append('Ошибка при открытии IDB: ' + e.target.errorCode + '.<br />');
	},
	onIDBRequestSuccess: function onIDBRequestSuccess(e) {
		app.db = e.target.result;
		app.debug.append('IDB открыта :)<br />');
	},
	resizeVideo: function resizeVideo() {
		var width = $('.intro-video').width();
		$('.intro-video').css('height', width / 1.7771 + 'px');
	},
	switchSchedule: function switchSchedule(e) {
		// console.dir(e);
	...
	showErrorPopup: function showErrorPopup(xhr, status, msg) {
		var title = 'Что-то пошло не так :(';
		var text = status + ' ~~~ ' + msg; //|| 'Попробуйте заполнить форму снова, и если проблема не решится, обязательно сообщите о ней нашим организаторам в VK.';
		var popupHtml = '\n\t\t\t<div class="popup-bg">\n\t\t\t  <div class="popup">\n\t\t\t    <div class="popup-inner">\n\t\t\t      <strong class="popup-title">\n\t\t\t        ' + title + '\n\t\t\t      </strong>\n\n\t\t\t      <p class="popup-text">' + text + '</p>\n\n\t\t\t      <div class="popup-socials">\n\t\t\t        <a href="https://www.youtube.com/channel/UCMJxMFJsR3ILLWjr8e1ToIQ" title="Канал Хакатона ДГТУ на YouTube" class="youtube" target="_blank"><i class="fa fa-youtube"></i></a>\n\n\t\t\t        <a href="https://vk.com/hackathon_dstu" title="Группа Хакатона ДГТУ в VK" class="vk" target="_blank"><i class="fa fa-vk"></i></a>\n\t\t\t      </div>\n\n\t\t\t      <div class="popup-close-btn" id="popup-close-btn"><i class="fa fa-close"></i></div>\n\t\t\t    </div>\n\t\t\t  </div>\n\t\t\t</div>\n\t\t';

		$('body').append(popupHtml);
		$('#popup-close-btn').click(app.closePopup);
	},
	showSuccessPopup: function showSuccessPopup(data, status) {

		var name = data.name;
		// var email = data.email;
		var title = 'Регистрация прошла успешно!';
		var text = 'благодарим за регистрацию и ждём 25 мая в 10:00 на Хакатоне!';

		var popupHtml = '\n\t\t\t<div class="popup-bg">\n\t\t\t  <div class="popup">\n\t\t\t    <div class="popup-inner">\n\t\t\t      <strong class="popup-title">\n\t\t\t        ' + title + '\n\t\t\t      </strong>\n\n\t\t\t      <p class="popup-text">' + name + ', ' + text + '</p>\n\n\t\t\t      <div class="popup-socials">\n\t\t\t        <a href="https://www.youtube.com/channel/UCMJxMFJsR3ILLWjr8e1ToIQ" title="Канал Хакатона ДГТУ на YouTube" class="youtube" target="_blank"><i class="fa fa-youtube"></i></a>\n\n\t\t\t        <a href="https://vk.com/hackathon_dstu" title="Группа Хакатона ДГТУ в VK" class="vk" target="_blank"><i class="fa fa-vk"></i></a>\n\t\t\t      </div>\n\n\t\t\t      <div class="popup-close-btn" id="popup-close-btn"><i class="fa fa-close"></i>\n\t\t\t      </div>\n\t\t\t    </div>\n\t\t\t  </div>\n\t\t\t</div>\n\t\t';

		$('body').append(popupHtml);
		$('#popup-close-btn').click(app.closePopup);
	},
	closePopup: function closePopup(e) {
		$('.popup-bg').addClass('remove-popup');

		setTimeout(function () {
			$('.popup-bg').remove();
		}, 450);
	},
	scrollToRegister: function scrollToRegister(e) {

		e.preventDefault();

		var dest = $("#register-form-top").offset().top;
		// console.log(dest);
		$('body').animate({ scrollTop: dest }, 1000);
	},
	onWindowScroll: function onWindowScroll(e) {

		if (app.clientWidth < 500) return;

		var scrolled = document.body.scrollTop;
		document.querySelectorAll('.header-wr')[0].style.setProperty('background-position', 'center ' + scrolled / 3 + 'px');
	},
	register: function register(e) {

		if (!$('#register-form')[0].checkValidity()) return;
		...
		var data = {
			...
			tel: $('#reg-tel')[0].value,
			edu: $('#reg-edu')[0].value,
			additional: $('#reg-additional')[0].value
		};

		if (!app.idb) {
			app.idb = window.indexedDB;
		}

		$.ajax('http://fantastic-hackathon.ru/action.php', {
			method: 'POST',
			data: data,
			statusCode: {
				404: function _() {
					app.debug.append('Page not found<br />');
				},
				403: function _() {
					app.debug.append('Forbidden<br />');
				},
				500: function _() {
					app.debug.append('Internal server error<br />');
				},
				200: function _() {
					app.debug.append('200 OK!<br />');
				}
			},
			success: app.showSuccessPopup,
			error: app.showErrorPopup
		});
